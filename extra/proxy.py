...
# Extract the People also ask questions from the first two search results
for url in urls[:2]:
    # Send a request to the URL and get the HTML response
    response = requests.get(url, headers=headers, proxies=proxies)
    soup = BeautifulSoup(response.text, 'html.parser')

    # Find the People also ask questions
    paas = soup.select('.related-question-pair')

    # Print the questions
    print("Questions from", url)
    for paa in paas[:2]:
        question = paa.select_one('.related-question').text
        print(question)
